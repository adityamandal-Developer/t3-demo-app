  ...
  async ({ event, step }) => {
    ...
    try {
      ...
      if (credits > 0) {
        ...
        await step.fetch(env.PROCESS_VIDEO_ENDPOINT.trim(), {
          method: "POST",
        ...
        const { clipsFound } = await step.run(
          ...
          async () => {
            ...
            const clipKeys = allKeys.filter(
              (key): key is string =>
                key !== undefined && !key.endsWith("orignal.mp4"),
            );
        ...
        await step.run("set-status-procesed", async () => {
          await db.uploadedFile.update({
            where: {
              id: uploadedFileId,
            },
...
async function listS3ObjectsByPrefix(prefix: string) {
  ...
  const respose = await s3Client.send(listCommand);
  return respose.Contents?.map((item) => item.Key).filter(Boolean) ?? [];
}
